...
export default function RoomView({ roomId, myPeerInfo, peers, sendFiles, leaveRoom }) {
  ...
  return (
    ...
    >
      ...
      {/* Drag & Drop Overlay */}
      {isDragOver && (
        <div className="fixed inset-0 z-50 bg-lime-950/90 backdrop-blur-xl border-4 border-dashed border-lime-400 flex flex-col items-center justify-center pointer-events-none animate-pulse px-4 text-center">
          ...
          <p className="text-lime-200 mt-1 text-xs sm:text-base">Direct WebRTC stream will start immediately</p>
        </div>
      )}
      ...
      <div className="w-full max-w-4xl glass-panel p-3.5 sm:p-4 rounded-2xl border border-slate-800/90 flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 shadow-xl mb-4">
        <div className="flex items-center justify-between sm:justify-start gap-3">
          <div className="flex items-center gap-2.5">
            <div className="p-2 rounded-xl bg-lime-500/10 border border-lime-500/20 text-lime-400 shrink-0">
            ...
            <div>
              ...
              <span className="font-mono font-extrabold text-lg sm:text-xl text-lime-300 tracking-wider">{roomId}</span>
            </div>
          </div>
        </div>
      ...
      <div className="flex-1 w-full max-w-4xl flex flex-col items-center justify-center relative my-2 min-h-[300px]">
        ...
        <div className="absolute w-[240px] h-[240px] sm:w-[420px] sm:h-[420px] rounded-full border border-lime-500/10 pointer-events-none" />
        <div className="absolute w-[170px] h-[170px] sm:w-[300px] sm:h-[300px] rounded-full border border-orange-500/10 pointer-events-none" />
        <div className="absolute w-[100px] h-[100px] sm:w-[160px] sm:h-[160px] rounded-full border border-slate-800 pointer-events-none" />

        {/* Pulse Radar rings */}
        <div className="absolute w-48 h-48 sm:w-64 sm:h-64 rounded-full border border-lime-500/20 animate-radar pointer-events-none" />
        <div className="absolute w-48 h-48 sm:w-64 sm:h-64 rounded-full border border-orange-500/20 animate-radar-delayed pointer-events-none" />

        {/* Center Node (Your Device) */}
        <div className="relative z-20 flex flex-col items-center text-center my-4">
          <div className="relative group">
            <div className="w-14 h-14 sm:w-20 sm:h-20 rounded-2xl bg-gradient-to-tr from-lime-500 to-green-600 p-0.5 sm:p-1 shadow-xl shadow-lime-500/30">
              <div className="w-full h-full bg-slate-950 rounded-[12px] sm:rounded-[14px] flex items-center justify-center text-lime-400">
        ...
        {peers.length === 0 ? (
          <div className="relative z-10 text-center max-w-xs sm:max-w-sm p-4 sm:p-5 rounded-2xl bg-slate-900/70 border border-slate-800/80 mt-2 backdrop-blur-md">
            <div className="w-8 h-8 mx-auto rounded-lg bg-lime-500/10 text-lime-400 flex items-center justify-center mb-2">
            ...
            <button
              onClick={() => setShowQR(true)}
              className="mt-3 px-3.5 py-1.5 rounded-xl bg-lime-500/10 hover:bg-lime-500/20 text-lime-400 text-xs font-semibold inline-flex items-center gap-1.5 transition-colors"
            >
              ...
              return (
                <div
                  ...
                  className={`glass-card p-3.5 sm:p-4 rounded-2xl border cursor-pointer relative transition-all ${
                    isSelected
                      ? 'border-lime-400 bg-lime-950/30 shadow-lg shadow-lime-500/20'
                ...
                >
                  ...
                  <div className="mt-3 pt-2.5 border-t border-slate-800/80 flex items-center justify-between text-xs">
                    ...
                    <button
                      ...
                      className="px-2.5 py-1 rounded-lg bg-lime-500/10 hover:bg-lime-500/20 text-lime-400 font-semibold text-[11px] flex items-center gap-1 transition-colors"
                    >
      ...
      <div className="w-full max-w-3xl glass-panel p-3 sm:p-4 rounded-2xl border border-slate-800 flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 relative z-30 shadow-2xl mt-3">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="w-full sm:w-auto py-2.5 px-4 rounded-xl font-semibold text-xs bg-gradient-to-r from-lime-500 to-orange-600 hover:from-lime-400 hover:to-orange-500 text-slate-950 shadow-lg shadow-lime-500/20 flex items-center justify-center gap-2 transition-all disabled:opacity-50 active:scale-[0.98]"
          >
